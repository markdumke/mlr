...
makeRLearnerInternal = function(id, type, package, par.set, par.vals, properties,
  ...
  assertString(id)
  assertChoice(type, choices = c("classif", "regr", "multilabel", "surv",
    "cluster", "costsens", "reinfLearn"))
...
#' @export
#' @rdname RLearner
makeRLearnerReinfLearn = function(cl, package, par.set, par.vals = list(), properties = character(0L), name = cl, short.name = cl, note = "", callees = character(0L)) {
  addClasses(
    makeRLearnerInternal(cl, "reinfLearn", package, par.set, par.vals, properties, name, short.name, note, callees),
    c(cl, "RLearnerReinfLearn")
  )
}
